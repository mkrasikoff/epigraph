 ...
 * (AuthService.verify / OAuth2SuccessHandler → QuoteService.createDefaultQuotes).
 *
 ...
 *
 * The three quotes come back newest-first with descending {@code added} offsets (base+2, base+1,
 * base) so the "welcome" card sorts to the very top of a brand-new collection.
 */
public final class OnboardingQuotesFactory {

    private static final String AUTHOR = "Epigraph";
    private static final String TAG = "инструкция";
     ...
     */
    public static List<Quote> build(long baseTimestamp) {
        return List.of(
    ...
    private static Quote quote(String text, long added) {
        Quote q = new Quote();
